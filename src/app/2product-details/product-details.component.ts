import { Component,AfterViewInit, OnInit, PLATFORM_ID, Inject, afterNextRender } from '@angular/core';
      ...
      this.httpClient.get(`https://friday.kubona.ng/api/RecentlyViewed?userId=${userId}&pageSize=5`).subscribe({
        next: (response: any) => {
          ...
          if (this.isRecentlyViewed) {
            setTimeout(() => this.initializeCarousel(), 0); // Initialize carousel if there are recently viewed items
          }
        },
        error: (error) => {
...
addToCartOld(){
  ...
  initializeCarousel2(){
    ...
    sync1.owlCarousel({
      ...
      responsiveRefreshRate: 200,
      navText: [
        '<img src="assets/images/ban-left.png" class="md:max-w-[45px] max-w-[35px] invert-[1] absolute left-0 top-[50%] translate-y-[-50%]" alt="Prev">',
        '<img src="assets/images/ban-right.png" class="md:max-w-[45px] max-w-[35px] invert-[1] absolute right-0 top-[50%] translate-y-[-50%]" alt="Next">'
      ],
